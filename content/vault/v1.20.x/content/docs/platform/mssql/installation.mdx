...
If restoring a backup encrypted with the EKM provider, refer to steps 1 through 3.
...
1. You can now create an asymmetric key using the transit key set up earlier:
  ...
  Above uses the latest transit key version. If your transit key has been rotated you 
  can pin a specific transit key version. This allows you to encrypt or restore a database 
  using and older key version: 
