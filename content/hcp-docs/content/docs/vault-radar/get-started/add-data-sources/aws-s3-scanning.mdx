...
| S3 (optional) | `s3:GetObjectVersion` | [GetObject API](https://docs.aws.amazon.com/AmazonS3/latest/API/API_GetObject.html) |
| S3 (optional) | `s3:ListBucketVersions` | [ListObjectVersions API](https://docs.aws.amazon.com/AmazonS3/latest/API/API_ListObjectVersions.html) |
...
The `s3:GetBucketPolicyStatus`, `s3:GetBucketAcl`, `s3:GetBucketPublicAccessBlock` and `s3:GetObjectAcl` permissions are also optional - the scan works without them but does not include appropriate visibility values of the bucket or the detected secrets.
